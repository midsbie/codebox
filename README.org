...
=cbox= can be invoked via symlink; it resolves its own location to find =compose.yaml=.
...
The image is built with your UID/GID baked in. Mounted volumes (repo, config) have correct ownership. Each user needs their own image (tagged =codebox:$USER=).
...
Git commit signing (GPG/SSH) does not work inside the container. The recommended workaround is to run a separate Claude instance on the host for reviewing and committing changes.

*** Editor integration

A shared temp directory (=/tmp/claude-editor-tmp=) is created on the host and mounted into the container as =$TMPDIR=. This allows a host editor (e.g. Emacs via =emacsclient=) to open temporary files created by Claude inside the container.
...
- Volta and Node.js LTS are pre-installed in the image
