...
Example on sharing the host NuGet cache so =dotnet restore= inside the container resolves packages instantly without a network round-trip:
...
- Accidental system damage; e.g. =rm -rf /=
- Accessing other repositories
- Reading =~/.ssh/id_*= private keys or other sensitive user information
- Accessing =~/.gnupg/=, password stores, etc.
- Modifying =/etc/= or system packages

*** What it does NOT protect

- Code execution by a determined attacker
- With =CBOX_CLIPBOARD=1=: X11 keylogging, screenshots, input injection
